...
* </ins>Running my [software.sh](Setup_files/software.sh) across all my servers allowed me to save time from separately downloading my app's dependencies on each server:</ins>
  	...
  	<ins>Along with installing the latest version of Python and the virtual environment, it downloaded my GitHub repo and installed the necessary dependencies in my repo's directory:</ins>
  		...
  		* pip install -y gunicorn *(automatically confirms and installs Gunicorn web server)*
  
  		* pip install -y mysqlclient *(automatically installs client library necessary for my SQLite database)*
  
  		* python -m gunicorn app:app -b 0.0.0.0 -D *(runs the Gunicorn server and binds it to all available network interfaces attached to my VPC, specify entry point and endpoint for the application, and runs Gunicorn as a daemon process)*
...
* </ins>For my RDS Database to manage and store my data, I manually edited my database files to create a database engine to connect to the relational database on AWS:</ins>
...
* **endpoint** to locate the database system where my SQLite database file will not be stored and used to return data being requested by a client

* **RDS Database name** so my code could find the correct relational database in AWS
  
**Database files**: 
*[load_data.py](load_data.py)*: utilized SQL to grab data from the database along with Flask and Bcrpyt for flexibility in creating rest API's, microservices, password encryption, and loads data to return it back to the client.
...
* My load balancer is essential in this deployment because it can properly distribute traffic evenly across my 4 application servers, and aids in lowering latency and downtime. Having more servers for users allows my servers more capacity to ensure proper configuration of the necessary applications and dependencies to deploy the application and in return creates a positive user experience.

* My RDS Database ensures fewer performance issues from running out of disk space as new data is stored from users accessing my application.
...
* Including "aws_autoscaling_policy" resource in Terraform or utilizing Kubernetes' autoscaler component to scale up or down as needed (ex. when resources like CPU reach a certain utilization), detect unhealthy instances and replace them, and automate recovery by redeploying failed instances.
  
* Create webhook through GitHub or Kubernetes to automatically trigger Jenkins build when there are changes to my GitHub repository to detect if any changes disrupt or optimize my deployment, reduce the risk of latency, and fix bugs for faster deployments. Additionally, I could use Lambda to potentially record any changes to my repo as an event that can trigger the function to run a new build in my Jenkins pipeline.
